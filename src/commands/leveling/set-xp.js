...
exports.execute = async (client, interaction) => {
	...
	const result = await LevelController.setXP(target.id, interaction.guild.id, amount);
	if (!result) {
	...
	if (member) {
		await LevelService.assignLevelReward(interaction.guild, member, result.level);
	}
